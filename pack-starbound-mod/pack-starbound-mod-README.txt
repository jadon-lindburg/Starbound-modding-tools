...
|  SETUP  |
-----------
1.	Copy pack-starbound-mod.cmd, pack-starbound-mod.ignore, and pack-starbound-mod.installpath to the folder that your mod folder is in.
...
4.	Copy the path into pack-starbound-mod.installpath, then save and close the file.
		...
		DO NOT surround it with quotes, otherwise pack-starbound-mod.cmd will not work, because pack-starbound-mod.cmd surrounds the install path with quotes itself when it runs.
...
5.	Put any filenames you do not want included in the .pak file into pack-starbound-mod.ignore (right click and select Edit).
		...
		Note that this is not a perfect solution and may cause files to be ignored that you do not want to be. For example, adding .cs\ to pack-starbound-mod.ignore would cause files named File.css or
		File.cs.txt to be ignored as well as ones with the extension .cs. Use this shortcut at your own risk. If files are missing from in-game after running pack-starbound-mod.cmd and you used
...
|   USE   |
-----------
To use pack-starbound-mod.cmd, just drag your mod folder onto it. As long as you have set everything up correctly, your mod will be packed directly into your Starbound\mods folder for testing
and into a folder named 'packed' in your MyStarboundMods folder for easy distribution.
For example, if your source files are in MyStarboundMods\ModName, pack-starbound-mod.cmd will save ModName.pak to both [installpath]\mods and MyStarboundMods\packed.

-----------
|  NOTES  |
-----------
If you do not want the temporary folder to stick around, edit pack-starbound-mod.cmd and copy this line (everything between the square brackets): [RD /s /q "packed\%foldername%\"] after the last line [pause].
This will cause pack-starbound-mod.cmd to automatically delete the temporary folder before closing.
...
If you do not want the command window to stay open, edit pack-starbound-mod.cmd and remove the last line [pause].
This will cause pack-starbound-mod.cmd to automatically close on completion.
